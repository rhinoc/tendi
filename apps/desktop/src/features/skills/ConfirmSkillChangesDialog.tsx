...
import { buildFileTreeRows, displayFileName, isJsonPath, isYamlPath, SkillChangeCommand } from "../../lib/index.ts";
...
export function ConfirmSkillChangesDialog({
  ...
  const previewLoadingClassName = command === SkillChangeCommand.UpdateMany
    ? "skillUpdatePreviewLoading"
    : "loadingStateCompact skillDeletePreviewLoading";
  return (
    <DialogShell
      open={open}
      onOpenChange={onOpenChange}
      className={command === SkillChangeCommand.DeleteMany ? "skillDeleteConfirmDialog" : undefined}
    ...
    >
      <div className="skillChangeDialogBody">
        ...
        {previewLoading && <LoadingState className={previewLoadingClassName} label={command === SkillChangeCommand.DeleteMany ? "Preparing deletion preview" : "Preparing update preview"} />}
      ...
      <div className="confirmDialogActions">
        ...
        {command === SkillChangeCommand.DeleteMany && (dependencies.length > 0 || dependents.length > 0) && onConfirmRelated && (
          <DialogStatefulButton
            ...
            loadingLabel="Deleting related skills"
            variant="danger-subtle"
            aria-label="Delete related too"
            onClick={onConfirmRelated}
          >
            Delete related too
          </DialogStatefulButton>
        )}
