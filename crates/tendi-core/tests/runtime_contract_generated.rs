...
use tendi_core::generated::runtime_contract::{
    CommandName, RuntimeEventEnvelope, validate_event, validate_request, validate_response,
    validate_result,
};

#[test]
fn generated_request_validator_enforces_schema_constraints() {
    assert!(validate_request("sessions_search", &json!({ "query": "session" }),).is_ok());
    assert!(
        validate_request(
            "sessions_search",
...
#[test]
fn generated_result_validator_accepts_nullable_session_group_count() {
    assert!(
        validate_result(
            "sessions_list",
            &json!({
...
fn generated_valid_and_invalid_fixtures_cover_rust_boundaries() {
    ...
    for (name, request) in fixtures["validRequests"]
        .as_object()
        .expect("valid requests")
        .iter()
    {
        assert!(
            CommandName::parse(name).is_some(),
            "unknown fixture command: {name}"
        );
        assert!(
            validate_request(name, request).is_ok(),
            "valid request rejected: {name}"
        );
    }
    for (name, request) in fixtures["invalidRequests"]
        .as_object()
        .expect("invalid requests")
        .iter()
    {
        assert!(
            validate_request(name, request).is_err(),
            "invalid request accepted: {name}"
        );
    }
    for (name, result) in fixtures["validResults"]
        .as_object()
        .expect("valid results")
        .iter()
    {
        validate_result(name, result)
            .unwrap_or_else(|error| panic!("valid result rejected: {name}: {error}"));
    }
    for (name, event) in fixtures["validEvents"]
        .as_object()
        .expect("valid events")
        .iter()
    {
        ...
        assert!(
            ...
            "valid event rejected: {name}"
        );
    }
    for (name, event) in fixtures["invalidEvents"]
        .as_object()
        ...
        .iter()
    {
        ...
        assert!(
            event.is_err() || validate_event(&event.expect("checked above")).is_err(),
            "invalid event accepted: {name}"
        );
    }
}
